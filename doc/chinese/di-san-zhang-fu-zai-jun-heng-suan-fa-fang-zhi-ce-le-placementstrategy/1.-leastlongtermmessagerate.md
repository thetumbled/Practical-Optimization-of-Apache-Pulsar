...
因为ThresholdShedder给broker打分是根据资源使用率来计算的，而LeastLongTermMessageRate是根据消息速率来打分的，这种**卸载和放置的标准不一致**很有可能导致反复的负载均衡执行，卸载出来bundle后却放到了错误的brokerX上，导致下一次执行shedding时又判定brokerX为超载。这也是后面提出新放置策略LeastResourceUsageWithWeight的动机。
